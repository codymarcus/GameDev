...
class CharacterMotorJumping {
        ...
        @System.NonSerialized
        var timeToWaitBetweenJumps : float = 0.1;
...
private function ApplyGravityAndJumping (velocity : Vector3) {
        ...
        {
                jumping.letGoOfJump = true;
        }
               
        // For some reason this get's turned off here
        if (!inputJump || !canControl) {
                jumping.holdingJumpButton = false;
                jumping.lastButtonDownTime = -100;
        }
       
        // If jump has been pressed, and the lastButtonDownTime is less than zero? It get's set to -100 in jumping
        if (inputJump && jumping.lastButtonDownTime < 0 && canControl)
                // ANd now we set it to the time we pressed the jump button, maybe becuase of the jump in the past buggy feeling
                jumping.lastButtonDownTime = Time.time;
       
        if (grounded)
        {
                // This is how Unity wants me to not fall throught the floor
                velocity.y = Mathf.Min(0, velocity.y) - movement.gravity * Time.deltaTime;
                jumping.letGoOfJump = false;
                jumping.currentJumps = 0;
        }
        else {
                // Otherwise it adds gravity
        ...
        if (grounded || jumping.letGoOfJump && inputJump && jumping.currentJumps < jumping.maxJumps) {
                if(jumping.letGoOfJump)
                {
                        jumping.currentJumps ++;
                        jumping.letGoOfJump = false;
                        jumping.lastButtonDownTime = Time.time;
                }
